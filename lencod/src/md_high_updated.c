/*!
 ***************************************************************************
 * \file md_high_updated.c
 *
 * \brief
 *    High Complexity RDO with an updated decision process.
 *    Unlike the original version, this gives a lower priority to 
 *    skip modes, to try and guarantee a bit better quality.
 *
 **************************************************************************
 */
...
*************************************************************************************
*/
void encode_one_macroblock_high_updated (Macroblock *currMB)
{
  ...
  for (currMB->c_ipred_mode = chroma_pred_mode_range[0]; currMB->c_ipred_mode<=chroma_pred_mode_range[1]; currMB->c_ipred_mode++)
  {
    ...
    {
      if (pslice)
        mode = mb_mode_table_updated[index];
      else
        mode = mb_mode_table[index];
      //printf("mode %d %7.3f\n", mode, (double) currMB->min_rdcost);
      if (enc_mb.valid[mode])
      {
        ...
        //if (mode == 0 && currMB->best_cbp == 0 )
          //printf("current best is %d and %.3f\n", currMB->best_mode, (double) currMB->min_rdcost);
...
//# 5: Enable only when best mode is inter with no residual or 16x16 with mvs close to skip (+-8)
        ...
        if ((pslice) && mode == 0)
        {
          MotionVector *allmvs = &currSlice->all_mv[0][0][0][0][0];
          switch (p_Inp->PSliceSkipDecisionMethod)
          {
            case 1:
              if( !(currMB->best_mode == 1 && currMB->cbp==0 &&
            ...
            case 3:
              if ( !(currMB->best_mode == 1 &&
                     motion[currMB->block_y][currMB->block_x].ref_idx[LIST_0]    == 0 &&
                     iabs(motion[currMB->block_y][currMB->block_x].mv     [LIST_0].mv_x - allmvs->mv_x) < 8 &&
                     iabs(motion[currMB->block_y][currMB->block_x].mv     [LIST_0].mv_y - allmvs->mv_y) < 8))
              {
                continue;
              }
              break;
            case 4:
              ...
              break;
            case 5:
              if ( !(currMB->cbp == 0 || (currMB->best_mode == 1 &&
                                          motion[currMB->block_y][currMB->block_x].ref_idx[LIST_0]    == 0 &&
                                          iabs(motion[currMB->block_y][currMB->block_x].mv     [LIST_0].mv_x - allmvs->mv_x) < 8 &&
                                          iabs(motion[currMB->block_y][currMB->block_x].mv     [LIST_0].mv_y - allmvs->mv_y) < 8)))
              {
